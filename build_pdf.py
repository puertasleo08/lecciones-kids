...
# Register Windows Segoe UI fonts for crisp, modern typography
...
def create_access_pdf(output_pdf_path):
    ...
    url = "https://drive.google.com/drive/folders/12j9LsjK1Ir7aTHw4WeMetJ6b68kloWD-"
    ...
    if os.path.exists(logo_path):
        logo_img = RLImage(logo_path, width=4.5*cm, height=2.25*cm)
    ...
    story.append(Paragraph(
        "Tu paquete completo de actividades y cuadernillos ya está listo en formato digital de alta calidad. Para acceder a tus archivos de forma inmediata, utiliza cualquiera de las dos opciones a continuación:",
        subtitle_style
    ))
    story.append(Spacer(1, 4*mm))

    # 4. Product Preview Card (Product mockup + feature highlights)
    capa_path = r"d:\dev\LECCIONES KIDS\imagens-pg-vendas\capa-produto.jpeg"
    if os.path.exists(capa_path):
        capa_img = RLImage(capa_path, width=2.7*cm, height=2.7*cm)
        product_bullets = [
            Paragraph("<b>Cuadernillos Matemáticas 4 Operaciones (1.º al 5.º Grado)</b>", product_title_style),
            Spacer(1, 1*mm),
            Paragraph("&bull; Archivos PDF en alta definición listos para imprimir", product_item_style),
            Paragraph("&bull; Adición, Sustracción, Multiplicación, División, Fracciones y más", product_item_style),
            Paragraph("&bull; Acceso permanente, vitalicio e ilimitado en la nube", product_item_style),
        ]
        prod_table = Table([[capa_img, product_bullets]], colWidths=[3.2*cm, 13.3*cm], hAlign='CENTER')
    ...
    # 6. Alternative Option Box (User's Exact Requested Text & Raw URL)
    ...
    url_clickable = Paragraph(
        f'<a href="{url}" color="#1D4ED8"><u><b>{url}</b></u></a>',
        url_text_style
    )
    url_tip = Paragraph(
    ...
    steps_data = [
        ...
        [
            ...
            Paragraph("Dentro de Google Drive verás todas las carpetas y archivos. Puedes abrirlos o descargarlos a tu equipo.", step_body_style),
    ...
    footer_p1 = Paragraph("<b>Acceso 100% Seguro e Ilimitado</b> • Material disponible las 24 horas para descargar cuando lo necesites.", footer_style)
    footer_p2 = Paragraph("¿Necesitas ayuda con tu descarga? Estamos para servirte en nuestros canales de atención.", footer_style)
    footer_p3 = Paragraph("<b>Lecciones Kids</b> • Materiales Educativos de Excelencia", ParagraphStyle('F3', parent=footer_style, fontName=FONT_BOLD, textColor=c_brown))
    ...
    doc.build(story)
    print(f"PDF built: {output_pdf_path}")

if __name__ == "__main__":
    out_pdf = r"d:\dev\LECCIONES KIDS\Acceso_al_Material.pdf"
    create_access_pdf(out_pdf)

    # Render preview
    p_doc = pdfium.PdfDocument(out_pdf)
    print(f"Total pages: {len(p_doc)}")
    page = p_doc[0]
    bitmap = page.render(scale=2)
    pil_image = bitmap.to_pil()
    preview_path = r"d:\dev\LECCIONES KIDS\preview_acceso.png"
